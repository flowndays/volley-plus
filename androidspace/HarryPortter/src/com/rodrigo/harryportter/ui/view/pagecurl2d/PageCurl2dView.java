...
public class PageCurl2dView extends View {

    private static final String TAG = "PageCurl2dView";
    ...
    public PageCurl2dView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init();
    }

    public PageCurl2dView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public PageCurl2dView(Context context) {
        super(context);
        init();
    }
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (e.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            mCurPageBitmap = mBitmapProvider.getCurrent();
            if (DragToRight()) {
                previous = mBitmapProvider.getPrevious();
                if (previous != null) {
                    mNextPageBitmap = previous;
                    mAnimation = true;
                } else {
                    return false;
                }
            } else {
                Bitmap next = mBitmapProvider.getNext();
                if (next != null) {
